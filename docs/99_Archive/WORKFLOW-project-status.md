...
### Local skill selection (`.agent/skills/`)
...
- See `.agentrules` for the detailed primary/companion skill map and route-specific constraints.
...
## 📝 Changelog
  ...
  ### April 5, 2026 - Bug Fixes & Automation Update
  
  - **Scope**
    - Fixed 500 Internal Server error in `/admin/research` API causing `Failed to fetch research projects`
    - Added missing `createdAt` and `publishedAt` column projections to `findAllAdmin` and `findAllPublic` in `ResearchService` to safely support Drizzle ORM `.orderBy()` conditions
    - Improved frontend fetch error logging and mapping in `AdminResearchPage`
    - Fully transitioned legacy `.windsurf` workflow references to standard `.agent` paths and created backup automation
  
  - **Impact**
    - The `/admin/research` page now reliably loads without SQL generator compilation faults
    - Client UI now provides explicit API error messages instead of generic catch boundaries
    - Agent workspace correctly synchronized across `docs` and execution artifacts
  ...
  - **Updated/New Files**
    ...
    - `.agent/workflows/research.md` - Created research workflow documentation
 ...
 - **Scope**
   ...
   - normalized the docs-governance skill and project review workflow to maintain only `.agent` workflow references
 ...
 - **Updated/New Files**
   ...
   - `.agent/skills/soc-crru-docs-governance/SKILL.md`
   - `.agent/skills/soc-crru-docs-governance/source-of-truth.md`
   - `.agent/skills/soc-crru-docs-governance/update-rules.md`
...
- **Scope**
  ...
  - consolidated executable workflows into `.agent/workflows/`
  - aligned documentation and workflow references across `docs/` and `.agent/workflows/`
  - fully migrated Chiang Rai Studies Admin Dashboard to use `shadcn/ui` components
  - established `.agentrules` as the primary Windsurf project rule file
  - aligned `README.md` and plan/workflow references with the Windsurf structure

- **Updated/New Files**
  - `.agentrules` (Primary Windsurf rules)
  - `README.md`
  - `docs/WORKFLOW-project-status.md`
 ...
 - **Scope**
   ...
   - corrected outdated workflow references from `.agent/workflows/` to `.agent/workflows/`

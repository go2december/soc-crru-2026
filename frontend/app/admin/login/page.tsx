...
import { Shield, ArrowLeft, Info, AlertCircle } from 'lucide-react';
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-900 dark:to-slate-950 flex items-center justify-center p-4">
            <Card className="w-full max-w-md shadow-xl border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
                ...
                <CardContent className="space-y-4">
                    {/* Error Message using shadcn-style alert */}
                    {error && (
                    ...
                    {/* Login Notice */}
                    <div className="bg-primary/5 text-primary text-sm p-3 rounded-md flex items-start gap-3 border border-primary/10">
                        <Info className="w-4 h-4 mt-0.5 shrink-0" />
                        <span className="leading-snug">
                            กรุณาเข้าสู่ระบบด้วยบัญชี Google ขององค์กร <strong>(@crru.ac.th)</strong> เท่านั้น
                        </span>
                    </div>
                    ...
                    <Button
                        ...
                        className="w-full h-11 text-base font-normal bg-white text-slate-700 hover:bg-slate-50 border border-slate-200 shadow-sm relative overflow-hidden group"
                        variant="outline"
                    >
                    ...
                    <Button
                        ...
                        variant="ghost"
                        size="sm"
                        className="w-full text-amber-600 hover:text-amber-700 hover:bg-amber-50 dark:text-amber-500 dark:hover:bg-amber-950/30"
                    >
                ...
                <CardFooter className="flex flex-col gap-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                    <Button
                        variant="link"
                        className="text-muted-foreground hover:text-primary gap-2 h-auto p-0"
                        onClick={() => router.push('/')}
                    >
                    ...
                    <div className="text-[10px] text-muted-foreground/50 text-center font-mono">
                        System Ready • {process.env.NODE_ENV}
                    </div>
                </CardFooter>
            </Card>
        </div>

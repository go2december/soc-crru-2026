...
export class NewsService {
    ...
    async findOne(id: string) {
        const result = await this.drizzle.db.select().from(news).where(eq(news.id, id));
        if (result.length === 0) throw new NotFoundException(`News with ID ${id} not found`);
        return result[0];
    }

    async findBySlug(slug: string) {
        const result = await this.drizzle.db.select().from(news).where(eq(news.slug, slug));
        if (result.length === 0) throw new NotFoundException(`News with slug ${slug} not found`);
        return result[0];
    }

...
export const staffProfiles = pgTable('staff_profiles', {
  ...
  academicPositionId: integer('academic_position_id').references(
    () => academicPositions.id,
  ),

  // ตำแหน่งบริหาร (ทั้งสายวิชาการและสนับสนุน ถ้ามี)
  adminPositionId: integer('admin_position_id').references(
    () => adminPositions.id,
  ),
...
export const researchProjects = pgTable(
  ...
  (table) => {
    return {
      ...
      isPublishedIdx: index('research_projects_is_published_idx').on(
        table.isPublished,
      ),
      fundingSourceIdx: index('research_projects_funding_source_idx').on(
        table.fundingSource,
      ),
      publishedAtIdx: index('research_projects_published_at_idx').on(
        table.publishedAt,
      ),
    };
  },
...
export const projectMembers = pgTable(
  'project_members',
  {
    ...
    staffProfileId: uuid('staff_profile_id').references(
      () => staffProfiles.id,
      {
        onDelete: 'set null',
      },
  ...
  (table) => {
    return {
      ...
      staffProfileIdx: index('project_members_staff_profile_id_idx').on(
...
export const researchOutputs = pgTable(
  ...
  (table) => {
    return {
      ...
      publicationDateIdx: index('research_outputs_publication_date_idx').on(
        table.publicationDate,
      ),
      outputTypeIdx: index('research_outputs_output_type_idx').on(
        table.outputType,
      ),
    };
  },
...
export const researchAttachments = pgTable(
  ...
  (table) => {
    return {
      projectIdx: index('research_attachments_project_id_idx').on(
        table.projectId,
      ),
      fileTypeIdx: index('research_attachments_file_type_idx').on(
        table.fileType,
      ),
      createdAtIdx: index('research_attachments_created_at_idx').on(
        table.createdAt,
      ),
    };
  },
...
export const admissionConfig = pgTable('admission_config', {
  ...
  tableTitle: varchar('table_title', { length: 255 }).default(
    'ตารางรอบรับสมัคร ประจำปีการศึกษา 2569',
  ),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
});
...
export const chiangRaiLearningSites = pgTable(
  ...
  (table) => {
    return {
      ...
      publishedAtIdx: index('cr_learning_sites_published_at_idx').on(
        table.publishedAt,
      ),
    };
  },

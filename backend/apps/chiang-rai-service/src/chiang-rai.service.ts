...
export class ChiangRaiService {
  ...
  async createActivity(data: typeof chiangRaiActivities.$inferInsert) {
    ...
    if (payload.eventEndDate && typeof payload.eventEndDate === 'string')
      payload.eventEndDate = new Date(payload.eventEndDate);
    return this.drizzle.db
      .insert(chiangRaiActivities)
      .values(payload)
      .returning();
  ...
  async getLearningSites(
      ...
      const sampleSites: (typeof chiangRaiLearningSites.$inferInsert)[] = [
        {
          ...
          description:
            'วัดศิลปะร่วมสมัยที่สร้างโดยอาจารย์เฉลิมชัย โฆษิตพิพัฒน์',
    ...
    if (searchQuery && searchQuery.trim().length > 0) {
      ...
      conditions.push(
        or(
          ilike(chiangRaiLearningSites.title, term),
          ilike(chiangRaiLearningSites.description, term),
          ilike(chiangRaiLearningSites.content, term),
        )!,
      );
    }
    ...
    const data = await query
      .orderBy(desc(chiangRaiLearningSites.publishedAt))
      .limit(limit)
      .offset(offset);
    const total = await countQuery;
  ...
  async getLearningSiteById(id: string) {
    const result = await this.drizzle.db
      .select()
      .from(chiangRaiLearningSites)
      .where(eq(chiangRaiLearningSites.id, id))
      .limit(1);
    if (!result.length)
      throw new NotFoundException(`Learning site ${id} not found`);
    return result[0];
  }
  ...
  async getLearningSiteBySlug(slug: string) {
    const result = await this.drizzle.db
      .select()
      .from(chiangRaiLearningSites)
      .where(eq(chiangRaiLearningSites.slug, slug))
      .limit(1);
    if (!result.length)
      throw new NotFoundException(`Learning site ${slug} not found`);
    return result[0];
  }
  ...
  async createLearningSite(data: typeof chiangRaiLearningSites.$inferInsert) {
    ...
    return this.drizzle.db
      .insert(chiangRaiLearningSites)
      .values(payload)
      .returning();
  }

  async updateLearningSite(
    id: string,
    data: Partial<typeof chiangRaiLearningSites.$inferInsert>,
  ) {
    const payload = { ...data };
    if (payload.publishedAt && typeof payload.publishedAt === 'string') {
      payload.publishedAt = new Date(payload.publishedAt);

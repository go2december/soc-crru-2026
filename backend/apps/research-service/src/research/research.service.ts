...
export class ResearchService {
  ...
  private async ensureStaffProfilesExist(members?: CreateProjectMemberDto[]) {
    const staffIds = Array.from(
      new Set(
        (members ?? []).map((member) => member.staffProfileId).filter(Boolean),
      ),
    ) as string[];
  ...
  private async getProjectMembersByProjectIds(projectIds: string[]) {
    ...
    return this.drizzle.db
      ...
      .leftJoin(
        staffProfiles,
        eq(projectMembers.staffProfileId, staffProfiles.id),
      )
  ...
  private normalizeExistingLocations(
    locations: any[],
  ): CreateProjectLocationDto[] {
  ...
  private normalizeExistingAttachments(
    attachments: any[],
  ): CreateResearchAttachmentDto[] {
  ...
  private async enrichProjectsForList(
    ...
    const [members, sdgs, attachments, outputs] = await Promise.all([
      ...
      projectIds.length
        ? this.drizzle.db
            .select({
              projectId: projectSdgs.projectId,
              sdgId: projectSdgs.sdgId,
            })
            .from(projectSdgs)
    ...
    const attachmentCountByProject = new Map<string, number>(
      attachments.map((item): [string, number] => [
        item.projectId,
        Number(item.count),
      ]),
    );
    const outputCountByProject = new Map<string, number>(
      outputs.map((item): [string, number] => [
        item.projectId,
        Number(item.count),
      ]),
    );
  ...
  private async getProjectDetailByCondition(condition: any) {
    ...
    const [members, locations, sdgs, outputs, attachments] = await Promise.all([
      ...
      this.drizzle.db
        ...
        .orderBy(
          asc(projectLocations.province),
          asc(projectLocations.district),
        ),
      this.drizzle.db
      ...
      this.drizzle.db
        ...
        .orderBy(
          desc(researchOutputs.publicationDate),
          asc(researchOutputs.id),
        ),
      this.drizzle.db
        .select()
        .from(researchAttachments)
        ...
        .orderBy(
          desc(researchAttachments.createdAt),
          asc(researchAttachments.id),
        ),
    ]);
  ...
  private async syncNestedCollections(
    ...
    await this.drizzle.db
      .delete(projectSdgs)
      .where(eq(projectSdgs.projectId, projectId));
    await this.drizzle.db
      .delete(researchOutputs)
      .where(eq(researchOutputs.projectId, projectId));
    await this.drizzle.db
      ...
      await this.drizzle.db.insert(researchOutputs).values(
        dto.outputs.map((output) => ({
          ...
          publicationDate: output.publicationDate
            ? new Date(output.publicationDate)
            : null,
  ...
  async findAllPublic(params: PublicListParams) {
    ...
    if (params.status)
      conditions.push(eq(researchProjects.status, params.status as any));
    if (params.fundingSource?.trim()) {
      conditions.push(
        eq(researchProjects.fundingSource, params.fundingSource.trim()),
      );
    }
    ...
    const projects = await this.drizzle.db
      ...
      .where(finalCondition)
      .orderBy(
        desc(researchProjects.year),
        desc(researchProjects.publishedAt),
  ...
  async findAllAdmin(params: AdminListParams) {
    ...
    const projects = (
      ...
      .orderBy(
        desc(researchProjects.updatedAt),
        desc(researchProjects.createdAt),
      )
      .limit(limit)
      .offset((page - 1) * limit);
  ...
  async exportAll(params: {
    ...
    if (params.q?.trim()) {
      const search = `%${params.q.trim()}%`;
      conditions.push(
        sql`(${researchProjects.titleTh} ILIKE ${search} OR ${researchProjects.titleEn} ILIKE ${search})`,
      );
    }
    if (params.year) conditions.push(eq(researchProjects.year, params.year));
    if (params.status)
  ...
  async getStats() {
    const [totals, topSdgsRaw, outputCountRow] = await Promise.all([
      ...
      this.drizzle.db
        .select({
          sdgId: projectSdgs.sdgId,
          count: count(projectSdgs.projectId),
        })
        .from(projectSdgs)
    ...
    return {
      ...
      outputCount,
      totalBudget,
      socialServicePercent: total
        ? Math.round((socialServiceCount / total) * 100)
        : 0,
      commercialPercent: total
        ? Math.round((commercialCount / total) * 100)
        : 0,
      topSdgs: topSdgsRaw.map((item) => ({
        sdgId: item.sdgId,
  ...
  async update(id: string, dto: UpdateResearchProjectDto) {
    ...
    const existingLocations = this.normalizeExistingLocations(
      existing.locations,
    );
    const existingOutputs = this.normalizeExistingOutputs(existing.outputs);
    const existingAttachments = this.normalizeExistingAttachments(
    ...
    if (dto.slug !== undefined && dto.slug !== existing.slug) {
      ...
      if (conflict.length) {
        throw new ConflictException(
          `Slug "${dto.slug}" ถูกใช้งานแล้วโดยโครงการอื่น`,
        );
      }
      nextSlug = dto.slug;
    }
    ...
    const result = await this.drizzle.db
      ...
      .set({
        ...
        slug: nextSlug,
        abstractTh:
          dto.abstractTh !== undefined
            ? dto.abstractTh?.trim() || null
            : existing.abstractTh,
        abstractEn:
          dto.abstractEn !== undefined
            ? dto.abstractEn?.trim() || null
            : existing.abstractEn,
        year: dto.year ?? existing.year,
        budget:
          dto.budget !== undefined ? (dto.budget ?? null) : existing.budget,
        fundingSource:
        ...
        publishedAt:
          dto.isPublished === false ? null : existing.publishedAt || new Date(),
        updatedAt: new Date(),
      })
    ...
    if (
      dto.coverImageUrl !== undefined &&
      existing.coverImageUrl !== nextCoverImageUrl
    ) {

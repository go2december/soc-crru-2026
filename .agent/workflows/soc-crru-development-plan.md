---
description: แผนงานพัฒนา Web Application คณะสังคมศาสตร์ CRRU (Education First, Social Innovator & Lifelong Learning)
---

# 🚀 แผนพัฒนาเว็บไซต์คณะสังคมศาสตร์ มรภ.เชียงราย (SOC-CRRU 2026)

Workflow นี้อ้างอิงจาก Sitemap Design วันที่ 9 มกราคม 2569 โดยเน้นผู้เรียนเป็นศูนย์กลาง (Learner-Centric)

## 📌 Phase 1: Foundation & Design System (รากฐานและดีไซน์)
- [x] **Setup Project**: Next.js (Frontend) + NestJS (Backend) + Docker
- [x] **Theme Setup**: Tailwind CSS v4 + DaisyUI + Scholar Palette (Theme "socTheme")
- [ ] **Global Layout**:
    - [ ] ปรับ **Navbar** ให้ตรงกับ Sitemap ใหม่ (เปลี่ยน "หลักสูตร" เป็น "การจัดการศึกษา (Academics)" และจัดกลุ่มเมนูตามโครงสร้างใหม่)
    - [ ] สร้าง **Footer** ที่มีข้อมูลติดต่อและ Social Links ครบถ้วน
    - [ ] สร้าง **Breadcrumb** component สำหรับนำทาง

## 🏠 Phase 2: Homepage (หน้าแรก)
- [ ] **Hero Banner**: ภาพบรรยากาศ Area-Based + คำโปรย Vision + ปุ่ม Action หลัก
- [ ] **Course Finder Section**: ระบบค้นหาหลักสูตร (ปริญญา/ระยะสั้น/ความสนใจ)
- [ ] **Lifelong Learning Highlight**: แบนเนอร์ Credit Bank และคอร์ส Upskill/Reskill
- [ ] **Social Impact Dashboard**: แสดง stats (ชุมชนที่ดูแล, นวัตกรรม)
- [ ] **News & Events**: ข่าวประชาสัมพันธ์และกิจกรรมล่าสุด

## 📚 Phase 3: Academics & Learning (การจัดการศึกษา)
- [ ] **Academics Landing Page**: ภาพรวมปรัชญาและ Learning Outcomes
- [ ] **Program Pages (Dynamic)**:
    - [ ] สร้าง Template สำหรับหน้ารายละเอียดหลักสูตร (จุดเด่น, โครงสร้าง, อาชีพ, อาจารย์)
    - [ ] หน้าแยกสาขา: พัฒนาสังคม, สังคมศาสตร์, คหกรรม, จิตวิทยา, GIS
- [ ] **Lifelong Learning Module**:
    - [ ] หน้า **Credit Bank**: ข้อมูลการสะสมหน่วยกิต
    - [ ] หน้า **Short Courses**: รายการคอร์สฝึกอบรม พร้อมระบบ Smart Card (ระบุรองรับ Credit Bank)

## 🎓 Phase 4: Admissions & Research (รับสมัคร & วิจัย)
- [ ] **Admissions Page**:
    - [ ] ข้อมูล TCAS และกำหนดการ
    - [ ] ระบบสมัครเรียนคอร์สระยะสั้น (Form หน้าเว็บ)
- [ ] **Research & Innovation**:
    - [ ] ฐานข้อมูลงานวิจัย (Searchable Table/List)
    - [ ] **Local Startups Showcase**: แสดงผลงานนวัตกรรมชุมชน

## 🏢 Phase 5: Organization & E-Service (องค์กร & บริการ)
- [ ] **About Us Pages**:
    - [ ] History & Vision timeline
    - [ ] Interactive Strategy Map (แผนยุทธศาสตร์)
    - [ ] **Staff Directory**: ระบบค้นหาอาจารย์และบุคลากร
- [ ] **E-Service Portal**: หน้ารวมลิงก์บริการ (นักศึกษา/บุคลากร)

## ⚙️ Phase 6: Backend & Database (NestJS + PostgreSQL)
- [ ] **Database Schema Design**: ออกแบบตาราง Users, Courses, Staff, Research, News
- [ ] **API Development**:
    - [ ] CRUD หลักสูตร (Courses)
    - [ ] CRUD บุคลากร (Staff)
    - [ ] CRUD ข่าวสาร (News)
- [ ] **Integration**: เชื่อมต่อ Frontend กับ API

## 🧹 Phase 7: Optimization & Launch
- [ ] **SEO Optimization**: ตั้งค่า Meta tags ตามหลักสูตร
- [ ] **Performance Tuning**: ทำ Image optimization และ Caching
- [ ] **Deployment**: ตรวจสอบ Docker Compose สำหรับ Production
